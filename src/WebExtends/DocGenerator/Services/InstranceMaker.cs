...
public static class InstanceMaker
{
    private static IList GetArrayWithInstanceInherts(Type itemType)
    {
        ...
        if (!itemType.IsAbstract) instance.Add(Make(itemType));
        foreach (var item in Assembly.GetEntryAssembly()?.GetTypes().Where(t => !t.IsAbstract)
                     .Where(t => t.IsSubclassOf(itemType)) ?? new List<Type>()) instance.Add(Make(item));
        return instance;
    }
    ...
    {
        if (type == typeof(string)) return "an example string.";
        ...
        if (type == typeof(DateTimeOffset) || type == typeof(DateTimeOffset?)) return DateTimeOffset.UtcNow;

        if (type == typeof(TimeSpan) || type == typeof(TimeSpan?)) return TimeSpan.FromMinutes(37);
        ...
        foreach (var property in instance.GetType().GetProperties())
            if (property.CustomAttributes.Any(t => t.AttributeType == typeof(JsonIgnoreAttribute)))
                ...
                property.SetValue(instance, null);
            else if (property.SetMethod != null) property.SetValue(instance, Make(property.PropertyType));
        return instance;
    }
}