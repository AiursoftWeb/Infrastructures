...
namespace Aiursoft.DocGenerator.Services
{
    public class MarkDownDocGenerator : ITransientDependency
    {
        ...
        private const string Authorized = "<span class=\"badge badge-pill badge-danger\"><i class=\"fa fa-shield\"></i> Authorize</span>";
        ...
        public string GenerateMarkDownForApi(IGrouping<string, API> docController, string apiRoot)
        {
            ...
            foreach (var docAction in docController)
            {
                ...
                content += "---------\r\n\r\n";
                content += $"<h3 id='{docAction.ActionName}'>{(docAction.IsPost ? Post : Get)} {(docAction.AuthRequired ? Authorized : string.Empty)} {docAction.ActionName.SplitStringUpperCase()}</h3>\r\n\r\n";
                // Request path.
