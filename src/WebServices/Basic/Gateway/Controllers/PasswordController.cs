...
public class PasswordController : Controller
{
    ...
    public async Task<IActionResult> EmailConfirm(string userId, string code)
    {
        ...
        if (mailObject == null || mailObject.OwnerId != user.Id || mailObject.Validated) return BadRequest();
    ...
    public async Task<IActionResult> EmailConfirmed(EmailConfirmViewModel model)
    {
        ...
        if (mailObject == null || mailObject.OwnerId != user.Id) return NotFound();
    ...
    public async Task<IActionResult> ForgotPasswordViaSms(ForgotPasswordViaEmailViewModel model)
    {
        ...
        var user = await _dbContext
            ...
            .Include(t => t.Emails)
            .SingleOrDefaultAsync(t => t.Id == mail.OwnerId);
        if (user.PhoneNumberConfirmed == false) return NotFound();
    ...
    public async Task<IActionResult> EnterSmsCode(string email)
    {
        ...
        var user = await _dbContext
            ...
            .Include(t => t.Emails)
            .SingleOrDefaultAsync(t => t.Id == mail.OwnerId);
        if (user == null || user.PhoneNumberConfirmed == false) return NotFound();
    ...
    {
        if (code == null) return RedirectToAction(nameof(ForgotPasswordFor));
    ...
    public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
    {
        ...
        if (result.Succeeded) return RedirectToAction(nameof(Confirmation));
        AddErrors(result);
        return View();
    }
